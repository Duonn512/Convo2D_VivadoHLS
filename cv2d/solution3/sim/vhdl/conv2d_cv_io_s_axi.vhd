-- ==============================================================
-- File generated on Thu Dec 19 02:17:10 +0700 2024
...
architecture behave of conv2d_cv_io_s_axi is
    ...
    constant ADDR_KERNEL_BASE   : INTEGER := 16#100#;
    constant ADDR_KERNEL_HIGH   : INTEGER := 16#13f#;
    ...
    signal int_kernel_address1 : UNSIGNED(3 downto 0);
    ...
    component conv2d_cv_io_s_axi_ram is
        generic (
            BYTES   : INTEGER :=4;
            DEPTH   : INTEGER :=256;
            AWIDTH  : INTEGER :=8);
    ...
    end component conv2d_cv_io_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_input_r
int_input_r : conv2d_cv_io_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 25,
     AWIDTH   => log2(25))
port map (
     ...
     q1       => int_input_r_q1);
-- int_kernel
int_kernel : conv2d_cv_io_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 9,
     AWIDTH   => log2(9))
port map (
     ...
     q1       => int_kernel_q1);
-- int_output_r
int_output_r : conv2d_cv_io_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 9,
     AWIDTH   => log2(9))
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_input_r_read = '0') and (int_kernel_read = '0') and (int_output_r_read = '0') else '0';
    RVALID    <= RVALID_t;
...
-- ----------------------- Memory logic ------------------
    -- input_r
    int_input_r_address0 <= UNSIGNED(input_r_address0);
    int_input_r_ce0      <= input_r_ce0;
    int_input_r_we0      <= '0';
    int_input_r_be0      <= (others => '0');
    int_input_r_d0       <= (others => '0');
    input_r_q0           <= STD_LOGIC_VECTOR(RESIZE(int_input_r_q0, 32));
    ...
    int_input_r_we1      <= '1' when int_input_r_write = '1' and WVALID = '1' else '0';
    int_input_r_be1      <= UNSIGNED(WSTRB);
    int_input_r_d1       <= UNSIGNED(WDATA);
    -- kernel
    int_kernel_address0  <= UNSIGNED(kernel_address0);
    int_kernel_ce0       <= kernel_ce0;
    int_kernel_we0       <= '0';
    int_kernel_be0       <= (others => '0');
    int_kernel_d0        <= (others => '0');
    kernel_q0            <= STD_LOGIC_VECTOR(RESIZE(int_kernel_q0, 32));
    int_kernel_address1  <= raddr(5 downto 2) when ar_hs = '1' else waddr(5 downto 2);
...
entity conv2d_cv_io_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity conv2d_cv_io_s_axi_ram;

architecture behave of conv2d_cv_io_s_axi_ram is
